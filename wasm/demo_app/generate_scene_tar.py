...
def add_robot_to_scene(scene_path, robot_xml_path, output_path=None, path_prefix="../"):
    ...
    if path_prefix and path_prefix != '../':
        ...
        post_process_xml_paths(output_path, path_prefix, robot_paths)
    
    print(f"\n✓ Saved scene to {output_path}")
    return output_path


def post_process_xml_paths(xml_path, path_prefix, robot_paths):
    """
    Post-process XML file to replace relative paths (../) with custom path prefix.
    Only replaces paths that are robot-related.
    
    Args:
        xml_path: Path to the XML file to process
        path_prefix: Custom path prefix to use (e.g., 'robots')
        robot_paths: Set of robot-related paths to replace
    """
    import re
    ...
    def replace_path(match):
        ...
        # Only add prefix if the path doesn't already start with it
        prefix = path_prefix.rstrip('/')
        if not relative_part.startswith(prefix):
            new_path = prefix + '/' + relative_part
        else:
            new_path = relative_part
